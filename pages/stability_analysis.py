...
def show():
    ...
    **Instrukcje:**
    ...
    - Na wykresie zostaną wyświetlone wszystkie serie wraz z liniami regresji.
    - Pod wykresem znajdziesz tabelę z parametrami regresji dla każdej serii.
    """)
        ...
        try:
            ...
            for col in series_columns:
                ...
                regression_results.append({
                    "Seria": col,
                    "Nachylenie": round(slope, 3),
                    "Wyraz wolny": round(intercept, 3),
                    "Współczynnik korelacji (r)": round(r_value, 3),
                    "Wartość p": f"{p_value:.3e}",
                    "Odchylenie standardowe": round(std_err, 3)
                })

            # Dodawanie linii specyfikacji
            if min_spec is not None:
                ax.axhline(min_spec, color='red', linestyle='--', label='Min Spec')
            if max_spec is not None:
                ax.axhline(max_spec, color='orange', linestyle='--', label='Max Spec')
            ...
            st.subheader("Wyniki analizy regresji dla poszczególnych serii")
